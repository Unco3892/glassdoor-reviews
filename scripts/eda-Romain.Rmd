...
jobs.dfm <- jobs.dfm[,1:100] #sort dfm by only keeping most used term for better display
...
reviews.bank.pro[i] <-merge.review.pro(reviews.complete, company.name[i])
}

for (i in 1:length(company.name)) {
reviews.bank.con[i] <-merge.review.con(reviews.complete, company.name[i])
}
...
# We tokenize and plot
review.tk <- tokens(review.cp, remove_numbers=TRUE, remove_punct=TRUE, remove_symbols=TRUE, remove_separators=TRUE)
review.tk <- tokens_tolower(review.tk) 
review.tk <- tokens_replace(review.tk, pattern=hash_lemmas$token, replacement = hash_lemmas$lemma)
review.tk <- review.tk %>% 
  tokens_remove(stopwords("english")) %>% tokens_remove("reuter")
review.sent <- tokens_lookup(review.tk, dictionary = data_dictionary_LSD2015) %>% dfm() %>% tidy()

ggplot(review.sent,aes(x=document, y=count, fill=term)) + geom_bar(stat="identity") + coord_flip()
