...
merge.review <- function(.Data, companyname) {
  .Data %>%
...
reviews.bank.merge <- as.character("","","","","")

for (i in 1:length(company.name)) {
reviews.bank.merge[i] <-merge.review(reviews.complete, company.name[i])
}


reviews.bank.merge <- corpus(x = reviews.bank.merge,
                             text_field = c("employer_pros"))

reviews.bank.merge <- tokens(reviews.bank.merge, 
                        remove_punct = TRUE, 
                        remove_symbols = TRUE, 
                        what="word")
...
reviews.bank.merge.sent <- reviews.bank.merge %>%
