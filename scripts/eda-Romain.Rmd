...
```{r}
textstat_lexdiv(reviews.bank.pro.dfm, measure = "I") %>% 
...
reviews.complete %>%
  group_by(company) %>%
  summarise(employer_rating = mean(employer_rating),
      ...
      overall_score=mean(employer_rating,
work_life_balance,
culture_values,
career_opportunities,
compensation_and_benefits,
senior_management)) %>%kable_maker()
```
...
reviews.complete <- separate(data = reviews.complete, col = employee_role, into = c(NA, "employee_role"), sep = "\\-")

jobs.corpus <- corpus(x = reviews.complete,
                             text_field = c("employee_role"))

jobs.tokens <- tokens(jobs.corpus, 
                        remove_punct = TRUE, 
                        remove_symbols = TRUE, 
                        what="word")
