...
scores_compared <- pos_sem %>% left_join(neg_sem, by = "company") %>% 
  mutate(afiin_senti_score = value.x+value.y) %>% 
  select(-c(2,3))

# Comparing the two scores
existing_score %>% left_join(scores_compared) %>% arrange(desc(2))
```
...
# Comparing the two scores
actual_score <- aggregate(employer_rating~company, data=bank_reviews, FUN=mean) %>% 
