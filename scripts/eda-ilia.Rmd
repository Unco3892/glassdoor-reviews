...
```{r}
# Two approach: Approach one do them separately
text_pos <- get_sentences(bank_reviews$employer_pros)
text_neg <- get_sentences(bank_reviews$employer_cons)

sentiments_pos <- sentiment(text_pos, question.weight = 0)
sentiments_neg <- sentiment(text_neg, question.weight = 0)

bank_reviews_head <- bank_reviews %>% head(100)
...
val_shifter(bank_reviews_head)


# Both semantics combined
overall_score <- sentiments_pos %>% left_join(sentiments_neg, by = "company") %>% mutate(score_numerical_analysis = value.x+value.y) %>% select(-c(2,3))

# Comparing the two scores
existing_score %>% left_join(overall_score, sentiments) %>% arrange(desc(2))

# Approach two is to do them together
