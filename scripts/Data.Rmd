...
* Data: JP Morgan, Deutsche Bank, TD, HSBC Holdings and UBS english reviews 

## Data Acquisition

Our process of data acquisition was composed of the following steps. First, we created a vector with the URL of the 5 desired companies. Next we define the minimum number of required reviews, which was 5010. After using a loop to go over the list of companies one by one and save the results in our tibble, we created a function to extract URLs. Finally, we remove any duplicates, parse the review time and finished by writing our CSV file with bank's processed reviews. 
