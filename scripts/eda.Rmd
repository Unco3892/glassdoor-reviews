...
#bank_reviews %>% filter(review_id == "empReview_38099527")
```
For unknown reasons, some reviews seem to have been duplicated. We remove these instances using filter.
...
# We plot the results
c<-review.sent %>% ggplot(aes(x = document, y = count, fill = term)) +
...
# We plot the results
d<-review.sent.scale %>% ggplot(aes(x = document, y = percent, fill = term)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  xlab("") +
  ylab("") +
