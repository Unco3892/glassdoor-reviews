...
```{r}
topic12_words <- data.frame(keyName = names(
  lsa.pros$features[, 12]),
  value = lsa.pros$features[, 12],
  row.names = NULL
)

topic12_words %>% arrange(desc(abs(value)))

```


```{r}
topic12_text <- data.frame(keyName = names(
  lsa.pros$docs[, 12]),
  value = lsa.pros$docs[, 12],
  row.names = NULL
)

topic12_text %>% arrange(value)

topic12_text %>% arrange(desc(value))




# most 2 associated to the topic
docvars(corpus.cons[7833])$employer_pros

docvars(corpus.cons[8839])$employer_pros

# least associated to topic
docvars(corpus.cons[12171])$employer_pros
```

Questions :
Balancing the Data
RMSE vs Accuracy
Impurity for understanding the topics
Interpretation of most important topics??
